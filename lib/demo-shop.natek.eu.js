...
async function scrapeProductHierarchy (elem) {
  try { 
    ...
    let categorryItemElems = rootHierarchyElem.children
    for (let i=0; i<categorryItemElems.length; i++) {
      let anchorElem = categorryItemElems[i].querySelector('a')
...
async function scrapePageForProductCatalogue () {
  ...
  try {
    ...
    let productElems = rootProductsElem.children
    for (let i=0; i<productElems.length; i++) {
      let anchorElem = productElems[i].querySelector('a')
